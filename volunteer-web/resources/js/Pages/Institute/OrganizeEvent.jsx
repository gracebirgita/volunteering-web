...
import {
    ...
    Sprout, UserRound, GraduationCap, Hospital, X,
} from "lucide-react";

// PROPS 'events' sekarang datang dari Controller Laravel
export default function MyEvent({ auth, events = [] }) {
    ...
    // FILTERING (Menggunakan data dari Backend)
    const filteredEvents = events.filter((event) => {
        // Filter Search (Menyesuaikan field 'event_name' dari BE)
        const matchSearch = (event.event_name || "")
            .toLowerCase()
            .includes(searchQuery.toLowerCase());

        // Filter Kategori (Field 'category' dari BE)
        const matchCategory = selectedCategory
            ? event.category === selectedCategory
            : true;

        // Filter Tanggal (Field 'event_start' atau 'date' dari BE)
        let matchDate = true;
        if (selectedDate) {
            // BE biasanya format YYYY-MM-DD, disesuaikan dengan selectedDate
    ...
    const getCategoryStyle = (cat) => {
        switch (cat) {
            ...
            case "Sosial": return "bg-[#FEEDE5] text-[#FF7A00]"; // Sesuaikan Social/Sosial
    ...
    const getCategoryIcon = (cat) => {
        switch (cat) {
            ...
            case "Sosial": return <UserRound size={14} />;
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                        <div className="lg:col-span-3">
                            {filteredEvents.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                    {filteredEvents.map((event) => (
                                        <div key={event.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col group hover:shadow-md transition">
                                            <div className="h-40 bg-gray-200 relative">
                                                <img
                                                    ...
                                                    className="w-full h-full object-cover"
                                                />
                                                <span className={`absolute top-3 right-3 px-3 py-1 rounded-full text-[10px] font-bold uppercase ${event.status === "Active" ? "bg-green-700 text-white" : "bg-red-500 text-white"}`}>
                                                    {event.status || "Active"}
                                                </span>
                                            </div>
                                            ...
                                            <div className="p-4 flex flex-col flex-1">
                                                <div className="flex gap-2 mb-3">
                                                    <span className={`flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold ${getCategoryStyle(event.category)}`}>
                                                ...
                                                <div className="mt-auto space-y-2 mb-4">
                                                    <div className="flex items-center gap-2 text-xs text-gray-600">
                                                        <Clock size={14} /> {event.event_start}
                                                    </div>
                                                    <div className="flex items-center gap-2 text-xs text-gray-600">
                                                        <MapPin size={14} /> 
                        ...
                        <div className="lg:col-span-1 space-y-8">
                            <div>
                                ...
                                <div className="space-y-3">
                                    {["Lingkungan", "Sosial", "Pendidikan", "Kesehatan"].map((cat, idx) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-full flex items-center gap-3 px-4 py-3 rounded-full text-xs font-bold transition border-2 
                                                ${isSelected ? `border-[#005D67] opacity-100 ${getCategoryStyle(cat)}` : `border-transparent hover:opacity-80 ${getCategoryStyle(cat)}`}`}
                                            >
                                                {getCategoryIcon(cat)} {cat}
                                            </button>
                                        );
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    placeholder="Ketik Lokasi"
                                    className="w-full bg-white border-none rounded-lg py-3 px-4 text-xs shadow-sm focus:ring-1 focus:ring-[#005D67]"
                                />
                            </div>
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    {selectedDate && (
                                        <button onClick={() => setSelectedDate("")} className="text-[10px] text-red-500 font-bold flex items-center hover:underline">
                                            <X size={12} className="mr-1" /> Reset
                                        </button>
                                    )}
                                </div>
