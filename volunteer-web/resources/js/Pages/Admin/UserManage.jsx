...
import { Search, Lock, AlertCircle, Menu } from "lucide-react";
import debounce from "lodash/debounce";

export default function UserManage({ auth, users, filters }) {
    const [activeTab, setActiveTab] = useState(filters.tab || "relawan"); 
    ...
    const debouncedSearch = useCallback(
        debounce((query, tab) => {
            router.get(route('manage.user'), { search: query, tab: tab }, {
                preserveState: true,
                preserveScroll: true,
                replace: true
            });
    ...
    const handleTabChange = (tab) => {
        ...
        router.get(route('manage.user'), { tab: tab }, { preserveState: true });
    };

    // --- Actions ---
    const handleToggle = (id) => {
        router.put(route('admin.user.toggle', id), {}, { preserveScroll: true });
    };

    const handleResetPassword = (id) => {
        if (confirm("Reset password to 'password123'?")) {
            router.put(route('admin.user.reset', id), {}, { preserveScroll: true });
        }
    };
                            ...
                            <table className="w-full text-left border-collapse min-w-[1000px]">
                                <thead className="bg-gray-100 sticky top-0 shadow-sm text-gray-800 z-10">
                                    <tr className="text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-gray-200">
                                        <th className="p-5">{activeTab === 'relawan' ? 'Nama Relawan' : 'Nama Organisasi'}</th>
                                        <th className="p-5">Email</th>
                                        <th className="p-5 text-center">Status</th>
                                        <th className="p-5 text-center">Aksi - Status</th>
                                        <th className="p-5 text-center">Reset Passowrd</th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr key={row.id} className="hover:bg-gray-50/50 transition-colors group">
                                            ...
                                            <td className="p-5 text-center">
                                                <div className="flex items-center justify-center gap-2">
                                                    ...
                                                    <button onClick = {() => handleResetPassword(row.id)}
                                                            className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 rounded-full text-xs font-semibold text-gray-600 hover:bg-gray-50 hover:text-red-600 transition-all shadow-sm">
